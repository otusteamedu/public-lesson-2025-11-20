...
5. Пробуем отправить любой запрос с ошибкой, видим ответ в `JSON` и новые записи для событий
6. В контейнере Redis по запросу `KEYS *` видим наш ключ `pl_app:builtinEvents`
7. По запросу `GET pl_app:builtinEvents` видим содержимое ключа со списком добавленных событий